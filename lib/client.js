...
IMAPClient.prototype.getNewMail = function(lastUID,callback){
    if(isNaN(this._selectedMailbox.UIDNext)){
        callback(new Error("Error UIDNext isNAN"));
    }

    if(!lastUID)
        lastUID = 0

    if (parseInt(this._selectedMailbox.UIDNext) - lastUID === 1){
        callback(null,null);
    }
    ...
    if(lastUID===0){
        this.listMessages(-2,callback)
    }else{
        this._send("UID FETCH "+lastUID+":* (UID FLAGS ENVELOPE BODY[HEADER.FIELDS (REFERENCES THREAD-INDEX)]" +(this._capabilities.indexOf("X-GM-EXT-1")>=0?" X-GM-LABELS X-GM-THRID":"")+ ")", (function(status){
            this._collectMailList = false;
