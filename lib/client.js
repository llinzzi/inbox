...
var X_CLIENT_NAME = "SeedMail",
    X_CLIENT_URL = "http://www.getseedmail.com";
 ...
 */
IMAPClient.prototype.GREETING_TIMEOUT = 45 * 1000;
...
/**
 * get the New Mail from lastUID
 *
 * @param {Number} the last UID
 * @param {Function} Callback
 */

IMAPClient.prototype.getNewMail = function(lastUID,callback){
    lastUID  = lastUID || 0;
    nextUID  = Number(this._selectedMailbox.UIDNext) || 0;
    boxCount = Number(this._selectedMailbox.count) || 0;
    startUID = 0;

    if(nextUID - lastUID === 1){
        callback(null,[]);
        return;
    }
    ...
    if(lastUID+nextUID==0 && boxCount>0){
        this.listMessages(-1,callback)
        return;
    }else if(lastUID==0 & nextUID>0){
        startUID = nextUID-5>0 ? nextUID-5 : 0
    }else if(lastUID>0){
        startUID = lastUID+1
    }

    if(startUID>0){
        this._send("UID FETCH "+startUID+":* (UID FLAGS ENVELOPE BODY[HEADER.FIELDS (REFERENCES THREAD-INDEX)]" +(this._capabilities.indexOf("X-GM-EXT-1")>=0?" X-GM-LABELS X-GM-THRID":"")+ ")", (function(status){
            this._collectMailList = false;
            if(typeof callback != "function"){
                return;
            }
            if(status == "OK"){
                ...
                callback(new Error("GetMail failed"));
            }
        }).bind(this));
        return;
   }else{
